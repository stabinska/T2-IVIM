%%  BiExpFitModelT2RelaxSegmented.m
%   This function contains a T2 corrected biexponential diffusion model
%   modelled as a 2D problem with segmented approach.
%
...
function fh = BiExpFitModelT2RelaxSegmented
...
%% Calculate forward jacobian matrix
    ...
    for SD = 1 : secDim                                                     % Loop over indirect dimension
        ...
        dYdDslow(:,SD) = S_0 * (- b_values .* exp(-D_slow * b_values) * exp(-(1 / T2_slow) * TE_values(SD)));
        
        
        % Partial derivative with respect to T2slow
        dYdT2slow(:,SD) = S_0 * (exp(-D_slow * b_values) .* (TE_values(SD) / T2_slow^2) .* ...
                       exp(-(1 / T2_slow) * TE_values(SD)));

        % Partial derivative with respect to S0
        dYdS0(:,SD) = exp(-D_slow * b_values) * exp(-(1 / T2_slow) * TE_values(SD));
    ...
    jac = cat(2, dYdS0, dYdDslow, dYdT2slow);   % Create final jacobian
...
%% Calculate forward model
    ...
    Y               = [];                                                   % Initialize model 
    for SD = 1 : secDim
        M = S_0*exp(-D_slow.*b_values)*exp(-(1/T2_slow).*TE_values(SD));
        Y = cat(1,Y,M);
    end
