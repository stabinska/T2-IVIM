...
function fh = BiExpFitModelT2RelaxS0
...
%% Define lossfunction, forward gradient, forward jacobian, forward model, x(2)pars, pars2x, fminunc_wrapper
...
%% Calculate forward jacobian matrix
    ...
    parStruct = x2pars(x,parameter.parametrization);                        % Convert x to readable parameters
    ...
    for SD = 1 : secDim                                                     % Loop over indirect dimension
        ...
        dYdS0(:,SD) = (1 - f_fast) * exp(-f_fast * b_values) * exp(-(1 / T2_slow) * TE_values(SD)) + ...
                 f_fast * exp(-D_slow * b_values) * exp(-(1 / T2_fast) * TE_values(SD));


    end                                                                     % End loop over indirect dimension

    
    if isfield(parameter, 'NoiseSD') && ~isempty(parameter.NoiseSD)         % Add noise standard deviation
        NoiseSD = parameter.NoiseSD;
    ...
    dYdffast = reshape((dYdffast./NoiseSD)',[],1);                          % Normalize f fast line
    dYdDslow = reshape((dYdDslow./NoiseSD)',[],1);                          % Normalize D slow line 
    dYdDfast = reshape((dYdDfast./NoiseSD)',[],1);                          % Normalize D fast line
    dYdT2slow = reshape((dYdT2slow./NoiseSD)',[],1);                        % Normalize T2 slow line
    dYdT2fast = reshape((dYdT2fast./NoiseSD)',[],1);                        % Normalize T2 fast line
    dYdS0     = reshape((dYdS0./NoiseSD)',[],1);                            % Normalize S 0 line

    jac = cat(2, dYdffast, dYdDslow, dYdDfast, dYdT2slow, dYdT2fast,dYdS0);   % Create final jacobian
...
%% Calculate forward model
    ...
    Y               = [];                                                   % Initialize model 
    for SD = 1 : secDim
        M = S_0*((1-f_fast)*exp(-D_slow.*b_values)*exp(-(1/T2_slow).*TE_values(SD)) ...
                + f_fast*exp(-D_fast.*b_values)*exp(-(1/T2_fast).*TE_values(SD)));
        Y = cat(1,Y,M);
    end
