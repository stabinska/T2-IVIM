%%  BiExpFitModelS0.m
%   This function contains a biexponential diffusion model
%
...
%% Define lossfunction, forward gradient, forward jacobian, forward model, D_slowpars, pars2x, fminunc_wrapper
...
%% Calculate forward jacobian matrix
    ...
    dYdDslow(:,1) = S_0 * (-(1 - f_fast) * b_values .* exp(-D_slow * b_values));
    ...
    dYdS0(:,1) = (1 - f_fast) * exp(-D_slow * b_values) + ...
    ...
    jac = cat(2, dYdffast, dYdDslow, dYdDfast,dYdS0);   % Create final jacobian
...
%% Calculate forward model
    ...
    % Initialize output
    Y = S_0*((1-f_fast)*exp(-D_slow.*b_values) + f_fast*exp(-D_fast.*b_values));
end
