function [out] = TerminalSeparate(outputDir,outFile,index_1,index_2,bset)


    load(fullfile(outputDir,outFile))
    PhantomPars.Model = 'BiExpFitModel';
    ...
    PhantomPars.NoiseSD= 2*PhantomPars.N_noise';
    ...
    switch bset
        ...
        case 2
            ...
            [~, indices_te_values, ~] = intersect(PhantomPars.TE_values, [47,52,57,62,67,72]);
         case 3
            [~, indices_b_values, ~] = intersect(PhantomPars.b_values, [0,30,70,100,200,750]);
            [~, indices_te_values, ~] = intersect(PhantomPars.TE_values, [47,52,57,62,67,72]);
        case 4
    ...
    DataMatrix = (DataMatrix(index_1:index_2,2,:,indices_b_values,indices_te_values,:,1,:,1,:));
    noise_reps = size(DataMatrix,1);
    

    f_fast = zeros(noise_reps,length(PhantomPars.TE(indices_te_values)),length(PhantomPars.TR),...
    ...
    if strcmp(PhantomPars.Model, 'BiExpFitModelT2RelaxS0')
        S_0 = f_fast;
        S_0_crlb = f_fast;
    end
    ...
    switch PhantomPars.Model
        ...
        case 'BiExpFitModelT2Relax'
            %         ffast Dslow   Dfast T2slow T2fast
            lb =      [ 0.01  0.0007  0.01 40 40 ];  % Biexp
            x0 =      [ 0.1  0.0015   0.08  40 40 ];
            ub =      [ 0.7 0.01    0.9  40 40 ];
    end
    
    
        for id_te = 1: length(PhantomPars.TE_values)
                        ...
                        for id_t2t = 1: length(PhantomPars.T2_tissue)
                            for id_t2b = 1: length(PhantomPars.T2_blood)                              
                                for id_rep = 1 : noise_reps    
                                    [xk,crlb] = createModelIVIM(squeeze(DataMatrix(id_rep,1,1,:,id_te,id_tr,id_t1t,id_t1b,id_t2t,id_t2b)), PhantomPars.Model,PhantomPars,lb,x0,ub,PhantomPars.NoiseSD(2,:),PhantomPars.TE_values(id_te));
                                    
                                   
                                    switch PhantomPars.Model
                                        case 'BiExpFitModelS0'
                                            f_fast(id_rep,id_te,id_tr,id_t1t,id_t1b,id_t2t,id_t2b) = xk(1);
                                            D_slow(id_rep,id_te,id_tr,id_t1t,id_t1b,id_t2t,id_t2b) = xk(2);
                                            D_fast(id_rep,id_te,id_tr,id_t1t,id_t1b,id_t2t,id_t2b) = xk(3);
                                            S_0(id_rep,id_te,id_tr,id_t1t,id_t1b,id_t2t,id_t2b) = xk(4);
                            
                                            f_fast_crlb(id_rep,id_te,id_tr,id_t1t,id_t1b,id_t2t,id_t2b) = crlb(1);
                                            D_slow_crlb(id_rep,id_te,id_tr,id_t1t,id_t1b,id_t2t,id_t2b) = crlb(2);
                                            D_fast_crlb(id_rep,id_te,id_tr,id_t1t,id_t1b,id_t2t,id_t2b) = crlb(3);
                                            S_0_crlb(id_rep,id_te,id_tr,id_t1t,id_t1b,id_t2t,id_t2b) = crlb(4);
    ...
    save(fullfile(outputDir,strrep(outFile,'withNoise.mat',['Phantom_Model_separate_Set_' num2str(bset) '_distribution_' num2str(index_2) '.mat'])),'Results','-v7.3');
    out = 1;
    exit;
end
